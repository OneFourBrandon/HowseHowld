...
export function ShareBar({
  ...
  if (!owed.length) {
    return (
      <div
        className={cn('rounded-full bg-(--raise)', className)}
        style={{ height }}
        aria-hidden="true"
      />
    )
  ...
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 grid place-content-center justify-items-center gap-0.5 text-center">
        ...
        {sublabel && <span className="text-[.66rem] font-semibold text-(--text-3)">{sublabel}</span>}
      </div>
    </div>
  )
}
    ...
    >
      <span className="relative">
        ...
        {showPaid && share.paidCents > 0 && (
          <span className="absolute -right-0.5 -bottom-0.5 grid size-3.5 place-items-center rounded-full bg-(--green) text-(--bg) shadow-[0_0_0_2px_var(--card)]">
      ...
      <span className="grid min-w-0 leading-tight">
        <span className={cn('truncate font-bold', compact ? 'text-[.8rem]' : 'text-[.86rem]')}>
          ...
          {share.isCurrentMember && <span className="font-medium text-(--text-3)"> (you)</span>}
        </span>
      ...
      <span className="grid justify-items-end leading-tight">
        <span
          ...
          style={{ color: share.member.color }}
        >
          {formatMoney(share.owedCents)}
        </span>
        <span className="text-[.7rem] font-semibold text-(--text-3) tabular-nums">
...
export function SharePeek({
  ...
  return (
    <div
      className={cn(
        'hh-card hh-anim-pop w-72 rounded-2xl p-3.5 shadow-[0_28px_60px_-18px_rgba(0,0,0,.85)]',
        className,
      )}
      role="tooltip"
    >
      ...
      {hidden > 0 && (
        <p className="pt-1.5 text-[.7rem] text-(--text-3)">+{hidden} more</p>
      )}
      <p className="mt-2 border-t border-(--line) pt-2 text-[.7rem] text-(--text-3)">
        {breakdown.mine
