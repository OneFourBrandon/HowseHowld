  ...
  return <main className="mx-auto grid min-h-svh w-full max-w-lg content-center gap-5 p-6">
    ...
    <form className="grid gap-4" onSubmit={async event => {
      ...
      try {
        ...
        const message = authMessage(cause)
        setError(mode === 'recover' && /share code is invalid/i.test(message)
          ? 'That recovery code is invalid or expired. Ask your admin for a new code.'
          : message)
      } finally { setBusy(false) }
    }}>
      {mode === 'join' || mode === 'recover' ? <>
        <label>{mode === 'recover' ? 'Recovery code' : 'House code'}<input autoComplete="off" value={code} onChange={event => setCode(event.target.value.toUpperCase())} disabled={busy} required /></label>
        {mode === 'join' && <label>Your name<input value={name} onChange={event => setName(event.target.value)} autoComplete="name" minLength={2} maxLength={80} disabled={busy} required /></label>}
      </> : sent ? <label>Six-digit code<input inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" value={token} onChange={event => setToken(event.target.value.replace(/\D/g, '').slice(0, 6))} disabled={busy} required /></label> : <label>Email address<input type="email" autoComplete="email" value={email} onChange={event => setEmail(event.target.value)} disabled={busy} required /></label>}
      ...
      {sent && <><Button type="button" variant="secondary" disabled={busy || wait > 0} onClick={async () => { setBusy(true); setError(''); try { await send() } catch (cause) { setError(authMessage(cause)) } finally { setBusy(false) } }}>{wait ? 'Resend in ' + wait + 's' : 'Resend code'}</Button><Button type="button" variant="ghost" disabled={busy} onClick={() => { setSent(false); setWait(0); setToken('') }}>Use another email</Button></>}
    </form>
    <button disabled={busy} className="text-sm text-(--forest-2)" onClick={() => { setMode('recover'); setPendingJoin(joining ? 'recover' : null); setSent(false); setWait(0); setError(''); setCode('') }}>Use a recovery code instead</button>
