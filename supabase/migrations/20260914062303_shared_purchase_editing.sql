drop function public.update_expense_details(uuid, integer, integer, jsonb);
drop function private.update_expense_details_impl(uuid, integer, integer, jsonb);
...
begin
  ...
  if actor is null then raise exception 'Not a household member'; end if;
  if p_amount is distinct from e.amount_cents and actor <> e.created_by then
    raise exception 'Only the creator can edit the purchase price';
  end if;
  ...
  if p_payers is not null then
    if jsonb_typeof(p_payers) <> 'array' then raise exception 'Payers must be a list'; end if;
    ...
    from jsonb_to_recordset(p_payers) as requested(member_id uuid, amount integer);
    if item_count < 1 or member_count <> item_count or minimum_share < 1 or share_total <> p_amount
      or exists (select 1 from jsonb_to_recordset(p_payers) as requested(member_id uuid, amount integer) where amount is null) then
      raise exception 'Payer amounts must be positive, unique, and equal the purchase total';
    end if;
    if exists (
      select 1 from jsonb_to_recordset(p_payers) as requested(member_id uuid, amount integer)
    ...
    ) then raise exception 'Every payer must be an active household member'; end if;
    payers := p_payers;
  end if;
  ...
  if p_beneficiaries is null then
    ...
    if item_count < 1 or member_count <> item_count or minimum_share < 1 or share_total <> p_amount
      or exists (select 1 from jsonb_to_recordset(p_beneficiaries) as requested(member_id uuid, amount integer) where amount is null) then
  ...
  delete from public.expense_payers where expense_id = e.id;
...
revoke all on function private.update_expense_details_impl(uuid, integer, integer, jsonb, jsonb) from public;
grant execute on function private.update_expense_details_impl(uuid, integer, integer, jsonb, jsonb) to authenticated;
...
set search_path = ''
as $$
  select private.update_expense_details_impl(p_expense_id, p_amount, p_expected, p_beneficiaries, p_payers)
$$;

revoke all on function public.update_expense_details(uuid, integer, integer, jsonb, jsonb) from public, anon;
grant execute on function public.update_expense_details(uuid, integer, integer, jsonb, jsonb) to authenticated;
